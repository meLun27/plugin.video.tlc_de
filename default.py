...
def listVideosMain(url, thumb):
    ...
    matchShowID = re.compile('id="dni_listing_post_id_e00903" value="(.+?)"', re.DOTALL).findall(content)
    ...
    matchClips = re.compile('<h2>KURZE CLIPS</h2>.+?id="listing-container-(.+?)"', re.DOTALL).findall(content)
...
def listAZ():
    ...
    for letter in letters:
        url = baseUrl+"/wp-content/plugins/dni_plugin_core/ajax.php?action=dni_listing_items_filter&letter="+letter.upper()+"&page=1&id=1b0&post_id=4797"
...
def playVideoAll(url, title, thumb):
    ...
    for part, videoID in matchMulti:
        ...
        if xbox:
            pluginUrl = "plugin://video/TLC.de/?url="+videoID+"&mode=playBrightCoveStream&isSingle=no&title="+urllib.quote_plus(title)+"&thumb="+urllib.quote_plus(thumb)
        else:
            pluginUrl = "plugin://plugin.video.tlc_de/?url="+videoID+"&mode=playBrightCoveStream&isSingle=no&title="+urllib.quote_plus(title)+"&thumb="+urllib.quote_plus(thumb)
